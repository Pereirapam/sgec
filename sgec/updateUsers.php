...
<body class="bg-gray-200 text-gray-600">
    ...
    <section>
        ...
        <div class="inline-flex items-center w-full px-5">
            ...
            <span class="absolute px-3 font-medium text-blue bg-gray-200 border border-gray-500 rounded-3xl left-36">Atualizar Users</span>
        ...
        <div class="flex justify-center items-center">
            <div class="bg-white px-4 rounded-lg shadow-md h-4/6 w-10/12 relative overflow-x-auto sm:rounded-lg">
                ...
                <div class=" flex justify-between items-center mb-6 mt-4">
                    <h1 class="text-2xl font-bold text-blue">Edite os dados do(a) Adiministrador(a)</h1>
                </div>

                <div class="w-full h-full">
                    <form class="flex flex-col justify-center w-full space-y-3" action="./configUsers/updateUsers.php" method="post" data-parsley-validate>
                        <div class="flex flex-col justify-center">
                            <input type="hidden" name="idUsers" value="<?php echo $user['idUsers']; ?>">
                            <label for="nome">Nome</label>
                            <input class="p-2 mt-2 rounded-xl border" type="text" name="nome" value="<?php echo $user['nome']; ?>" required>
                        </div>

                        <div class="flex flex-col md:flex-row justify-between items-center">
                            <div class="flex flex-col w-full md:w-6/12">
                                <label for="email">Email</label>
                                <input class="p-2 mt-2 rounded-xl border" type="email" name="email" value="<?php echo $user['email']; ?>" required>
                            </div>
                            <div class="flex flex-col w-full md:w-6/12 md:ml-2 mt-2 md:mt-0">
                                <label for="senha">Senha</label>
                                <input class="p-2 mt-2 rounded-xl border" type="text" name="senha" value="<?php echo $user['senha']; ?>" required>
                            </div>
                        </div>

                        <div class="flex justify-end items-center space-x-3">
                            <input type="submit" name="btn_update_users" class="bg-green text-white px-4 py-2 rounded font-bold" value="Atualizar">
                        </div>
                    </form>
                </div>
